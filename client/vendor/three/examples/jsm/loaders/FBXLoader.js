	...
	parseMeshGeometry( relationships, geoNode, deformers ) {
		...
		relationships.children.forEach( function ( child ) {
			...
			if ( morphTargets[ child.ID ] !== undefined ) {
				...
				morphTargets[ child.ID ].rawTargets.forEach( function ( rawTarget ) {

					if ( rawTarget.geoID === parseInt( relationships.parents[ 0 ].ID ) ) {

						morphTargets[ child.ID ].geoID = parseInt( relationships.parents[ 0 ].ID );
	...
	interpolateRotations( curvex, curvey, curvez, eulerOrder ) {
		...
		for ( let i = 1; i < curvex.values.length; i ++ ) {
			...
			if ( absoluteSpan[ 0 ] >= 180 || absoluteSpan[ 1 ] >= 180 || absoluteSpan[ 2 ] >= 180 ) {
				...
				const E1 = new Euler( ...initialValue, eulerOrder );
				const E2 = new Euler( ...currentValue, eulerOrder );
